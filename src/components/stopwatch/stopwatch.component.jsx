    ...
    if (!isRunning) {
      ...
      const newLap = {
        lapTime: `${formatTime(Date.now() - startLapTime)
          .slice(0, 3)
          .join(":")}.${formatTime(Date.now() - startLapTime).slice(3)}`,
        elapsedTime: `${formatTime(time).slice(0, 3).join(":")}.${formatTime(
          time
        ).slice(3)}`,
  ...
  useEffect(() => {
    if (time) {
      window.document.title =
        formatTime(time).slice(0, 3).join(":") + " | Stopwatch";
    } else {
  ...
  return (
    <div>
      ...
      <div className={"stopwatch-timer"}>
        ...
        <span className={"stopwatch-miliseconds"}>
          {formatTime(time).slice(3)}
        </span>
      </div>
      <h4 className={"lap-timer"}>
        {`${formatTime(lapTime).slice(0, 3).join(":")}`}
        <span className={"lap-miliseconds"}>
          {formatTime(lapTime).slice(3)}
        </span>
      </h4>
      <div className="button-container">
        ...
        <button
          onClick={handleStartPause}
          className={`${isRunning ? "" : "hidden"} pause-button button`}
        >
          Pause
        </button>
        <button
        ...
        <button
          ...
          className={`${isRunning ? "" : "hidden"} lap-button button`}
        >
          Lap
        </button>
        <button
      ...
      {lapsList.length > 0 && (
        <div>
          ...
          {/*Lap table*/}
          <div className="button-container">
            <button className={"clear-button button"} onClick={handleClearLap}>
              Clear Data
            </button>
          </div>
        </div>
